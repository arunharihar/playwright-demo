...
export class TeamQAInteractionsPage {
    private page: Page;
    ...
    //Locators
    browserWindows = async () => this.page.$('h5:has-text("Interactions")')
    ...
    //Actions
    public async clickInteractionsWindows() {
    ...
    public async verifyDragAndDrop(src: string, dst: string) {
        await delay(1000);
        await this.page.dragAndDrop(src, dst, { timeout: 3000, force: true })
        await delay(1000);
    }
}
